...
import { ContextManager, DID, SchemaManager } from '../common';
...
export async function verifyPresentationJWT(
    ...
    throw TypeError('Only JWT supported for Verifiable Presentations');
}
 ...
 */
export async function verifyDIDs(
    vc: VerifiableCredential,
    didResolver: Resolvable,
): Promise<boolean> {
    let issuer;
 ...
 */
export async function verifyRevocationStatus(
    vc: VerifiableCredential,
    didResolver: Resolvable,
): Promise<boolean> {
    let vcId;
