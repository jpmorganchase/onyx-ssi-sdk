...
import { DEFAULT_CONTEXT, EthrDIDMethod, JWTService, KeyDIDMethod, PROOF_OF_NAME, SCHEMA_VALIDATOR, VERIFIABLE_CREDENTIAL } from "../../../src/services/common"
import { createAndSignCredentialJWT, createCredential, createCredentialFromSchema, revokeCredential } from "../../../src/services/issuer/credential"
...
describe('credential utilities', () => {
    ...
    it('Successfully Creates Credential Object', async () => {
        const credential = createCredential(issuer.did, holder.did, subjectData, [PROOF_OF_NAME])
        ...
        expect(credential.credentialSubject).toEqual({...subjectData, id:holder.did})
    ...
    it('Successfully Creates Credential Object with additional properties', async () => {
        ...
        const credential = createCredential(issuer.did, holder.did, subjectData, [PROOF_OF_NAME], additionalParams)
        ...
        expect(credential.credentialStatus).toEqual(additionalParams.credentialStatus)
        expect(credential.credentialSchema).toBeDefined()
        expect(credential.credentialSchema).toEqual(additionalParams.credentialSchema)
        expect(credential.expirationDate).toBeDefined()
        expect(credential.expirationDate).toEqual(additionalParams.expirationDate)
        expect(credential.id).toBeDefined()
        expect(credential.id).toEqual(additionalParams.id)
    })  

    it('Successfully Creates Credential Object from Schema', async () => {
        const credential = await createCredentialFromSchema('schemaLocation', issuer.did, holder.did, subjectData, PROOF_OF_NAME)
        ...
        expect(credential.issuanceDate).toBeDefined()
        expect(credential.credentialSchema).toBeDefined()
        expect(credential.credentialSchema).toEqual({id: 'schemaLocation', type: SCHEMA_VALIDATOR})
    })  

    it('Successfully Creates and Signs VC JWT', async () => {

        const credential = await createAndSignCredentialJWT(
            issuer, holder.did, subjectData, [PROOF_OF_NAME])
    ...
    it('Revocation fails when Revocation transaction fails', async () => {
        ...
        ethrMethod.deactivate = jest.fn().mockRejectedValueOnce(
            new DIDMethodFailureError('Error deactivating'))
        await expect(revokeCredential(vc, ethrMethod))
    ...
    it('Revocation Fails with did method that doesnt support deactivation', async () => {
        ...
        await expect(revokeCredential(key, keyMethod))
            .rejects.toThrowError(DIDMethodFailureError)
    })
})